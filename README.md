...
> **범위(2026-09-08)**: 캐스토는 **사용자 개인 채널 전담**이다 — 유튜브 「콕픽」(@kokpick_kr) + 콕픽 인스타.
> 유행템 발굴 → 쇼츠 제작 → **쿠팡 파트너스 전환**까지가 캐스토의 일이다.
> 픽담·원더랜드로의 **글 유입은 캐스토의 일이 아니다** — 새 도구 **마크토(Markto)**가 전담한다.
> 캐스토가 픽담 쪽에 계속 제공하는 것은 유행템 글감 요청서(`data/topic_requests.json`) 하나뿐이며,
> 흐름은 **캐스토 → 픽토 → 마크토** 한 방향이다.
